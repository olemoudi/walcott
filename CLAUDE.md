...
- **The release key lives outside the repository** (see `docs/signing.md`): CI reads it from the `SIGNING_*` secrets, ole's machine from `walcott.signing.*` in `local.properties`. Without a key, release builds are unsigned and debug builds use the SDK debug key, which no Device Owner phone accepts. The original 2026-07 key was committed and is therefore public; it was rotated with APK Signature Scheme v3 and `signing/walcott.lineage` is the proof. **Every release must be signed with the current key AND the lineage** (`scripts/sign-apk.sh`, which CI runs) — a phone that took a rotated build refuses anything else, and re-signing with a fresh key without a lineage breaks the update chain and requires a factory reset of every child.
- Releases are published by GitHub Actions on pushing a tag matching `v*`. The workflow builds `assembleRelease` and attaches three assets with **stable names**: `walcott.apk`, `walcott-alpha.apk` (a byte-identical copy) and `version.json`.
- Stable URLs: APK at `…/releases/latest/download/walcott.apk` (in-app QR points here); `version.json` at `…/releases/latest/download/version.json`. Keep the names stable so old QRs and installed apps keep working.
- The canonical asset name carries **no release stage** on purpose (`walcott.apk`, not `-alpha`/`-beta`): it is baked into every onboarding QR ever shown, so it must outlive every stage. The stage is user-facing information and lives in `versionName` (e.g. `0.25.0-beta`), which nothing parses — only `versionCode` drives updates.
- **`walcott-alpha.apk` must keep being published**, identical bytes, even though nothing new points at it. It was the canonical name until 0.25.0, and a parent phone that hasn't updated yet still shows an onboarding QR encoding it; dropping it would 404 those scans. Auto-update never depended on either name — `Updater` follows the url inside `version.json` and only checks the host prefix.
