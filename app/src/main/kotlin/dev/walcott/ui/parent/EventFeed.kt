...
private val RENDERABLE_TYPES = setOf(
    ...
    ParentEvent.TYPE_BACK, ParentEvent.TYPE_LAST_GASP, ParentEvent.TYPE_LOST_MODE,
...
private fun eventBadge(event: ParentEvent): Pair<ImageVector, Color> {
    ...
    return when (event.type) {
        ...
        ParentEvent.TYPE_LAST_GASP -> Icons.Outlined.CloudOff to error
        ParentEvent.TYPE_LOST_MODE -> Icons.Outlined.LocationOff to if (event.count == 1) error else good
...
private fun eventText(event: ParentEvent, name: String): String? = when (event.type) {
    ...
    ParentEvent.TYPE_LAST_GASP -> stringResource(
        if (event.detail == dev.walcott.sync.LastGasp.KIND_SHUTDOWN) {
            R.string.event_last_gasp_shutdown
        } else {
            R.string.event_last_gasp_battery
        },
        name,
    )
    ParentEvent.TYPE_LOST_MODE -> stringResource(
        if (event.count == 1) R.string.event_lost_mode_on else R.string.event_lost_mode_off, name,
    )
